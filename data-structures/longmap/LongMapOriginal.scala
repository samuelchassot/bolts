...
/** This class implements mutable maps with `Long` keys based on a hash table with open addressing.
  *
  * Basic map operations on single entries, including `contains` and `get`, are typically substantially faster with `LongMap` than [[HashMap]]. Methods that act on the whole map, including `foreach` and `map` are not in general expected to be faster than with a generic map, save for those that take particular advantage of the internal structure of the map: `foreachKey`, `foreachValue`,
  ...
  */
final class LongMap[V] private[collection] (defaultEntry: Long => V, initialBufferSize: Int, initBlank: Boolean) extends AbstractMap[Long, V] with MapOps[Long, V, Map, LongMap[V]] with StrictOptimizedIterableOps[(Long, V), Iterable, LongMap[V]] with Serializable {
  import LongMap._
  ...
  /** Retrieves the value associated with a key. If the key does not exist in the map, the `defaultEntry` for that key will be returned instead.
    */
  ...
  /** Repacks the contents of this `LongMap` for maximum efficiency of lookup.
    ...
    */
  def repack(): Unit = {
    println(f"BEGIN repack, with oldMask = $mask, size = $size, vacant = $_vacant")
    var m = mask
    if (_size + _vacant >= 0.5 * mask && !(_vacant > 0.2 * mask)) m = ((m << 1) + 1) & IndexMask
    while (m > 8 && 8 * _size < m) m = m >>> 1
    println(f"END repack, new mask = $m")
    repack(m)
  }
  ...
  override def update(key: Long, value: V): Unit = {
    // println(f"Call update ($key -> $value), mask = $mask")
    if (key == -key) {
      if (key == 0) {
        zeroValue = value.asInstanceOf[AnyRef]
        extraKeys |= 1
  ...
  /** Creates a new `LongMap` with different values. Unlike `mapValues`, this method generates a new collection immediately.
    */
