...
case class ListMap[K, B](toList: List[(K, B)]) {
  ...
  def -(key: K): ListMap[K, B] = {
    ListMap(TupleListOpsGenK.removeNoDuplicatedKeys(toList, key))
...
object TupleListOpsGenK {
  ...
  def removeNoDuplicatedKeys[K, B](
      l: List[(K, B)],
      key: K
  ): List[(K, B)] = {
    require(invariantList(l))
    ...
    l match {
      ...
      case Cons(head, tl) if (head._1 != key) =>
        val res = head :: removeNoDuplicatedKeys(tl, key)
        if(getKeysList(tl).contains(head._1)){
          lemmaInGetKeysListThenContainsKey(tl, head._1) 
          check(false)
        }
        if(containsKey(removeNoDuplicatedKeys(tl, key), head._1)){
          lemmaInListThenGetKeysListContains(removeNoDuplicatedKeys(tl, key), head._1)
          check(false)
        }
        res
      case Nil() => Nil[(K, B)]()
  ...
  @opaque
  @inlineOnce
  def lemmaInsertAndremoveNoDuplicatedKeysCommutative[K, B](
      l: List[(K, B)],
      key1: K,
      v1: B,
      key2: K
    ...
    l match {
      case Cons(head, tl) => {
        lemmaInsertAndremoveNoDuplicatedKeysCommutative(tl, key1, v1, key2)
      }
      case _ => ()
    }
    ...
    insertNoDuplicatedKeys(
      removeNoDuplicatedKeys(l, key2),
      key1,
      v1
    ) == removeNoDuplicatedKeys(
  ...
  }.ensuring(_ => removeNoDuplicatedKeys(insertNoDuplicatedKeys(l, key1, v1), key1) == l)
  ...
  }.ensuring(_ => insertNoDuplicatedKeys(removeNoDuplicatedKeys(l, key1), key1, v1).content == insertNoDuplicatedKeys(l, key1, v1).content)
  ...
  @opaque
  @inlineOnce
  def lemmaremoveNoDuplicatedKeysCommutative[K, B](
      l: List[(K, B)],
      key1: K,
      key2: K
  ): Unit = {
    ...
    l match {
      case Cons(head, tl) => {
        lemmaremoveNoDuplicatedKeysCommutative(tl, key1, key2)
      }
      case _ => ()
    }
  ...
  @opaque
  @inlineOnce
  def lemmaremoveNoDuplicatedKeysNotPresentPreserves[K, B](
      l: List[(K, B)],
      key: K
  ): Unit = {
    require(invariantList(l))
    ...
    l match {
      case Cons(head, tl) => {
        ...
        lemmaremoveNoDuplicatedKeysNotPresentPreserves(tl, key)
      }
      case _ => ()
    }

  }.ensuring(_ => removeNoDuplicatedKeys(l, key) == l)
...
object ListMapLemmas {
  ...
  def removeNotPresentStillSame[K, B](lm: ListMap[K, B], a: K): Unit = {
    ...
    TupleListOpsGenK.lemmaremoveNoDuplicatedKeysNotPresentPreserves(lm.toList, a)
  }.ensuring(_ => lm - a == lm)
  ...
  def addRemoveCommutativeForDiffKeys[K, B](
    ...
    require(a1 != a2)
    TupleListOpsGenK.lemmaInsertAndremoveNoDuplicatedKeysCommutative(
      lm.toList,
      a1,
      b1,
      a2
  ...
  def removeCommutative[K, B](lm: ListMap[K, B], a1: K, a2: K): Unit = {
    TupleListOpsGenK.lemmaremoveNoDuplicatedKeysCommutative(lm.toList, a1, a2)
